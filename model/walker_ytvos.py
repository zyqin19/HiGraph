...
import numpy as np
import utils
from Deformable_layer import defom_sample
from model.rwgraph import RW_NN
from utils.rwgraph_utils import generate_batches, fea_to_graph
...
class CRW(nn.Module):
    def __init__(self, args, vis=None):
        ...
        self.defom = defom_sample

        self.rw_gk = RW_NN(128, args.batch_size, args.clip_len, args.max_step, args.hidden_graphs,
                           args.size_hidden_graphs, args.hidden_dim,
                           args.penultimate_dim, args.rw_normalize, args.n_classes,
                           args.rw_dropout, self.args.device).to(self.args.device)
    ...
    def pixels_to_nodes(self, x):
        ...
        # maps_backbone = self.encoder(x.flatten(0, 1))
        #
        # maps_b_B, maps_b_C, maps_b_T, maps_b_H, maps_b_W = maps_backbone.shape
        #
        # maps_backbone = maps_backbone.reshape(maps_b_B, maps_b_C*maps_b_T, maps_b_H, maps_b_W)
        #
        # maps = self.defom(maps_backbone, d_model=maps_b_C*maps_b_T)
        #
        # maps = maps.reshape(maps_b_B, maps_b_C, maps_b_T, maps_b_H, maps_b_W)
    ...
    def forward(self, x, label, just_feats=False, ):
        '''
        ...
        # Pixels to Nodes
        #################################################################
        x = x.transpose(1, 2).view(B, 1, C, T, H, W)
        ...
        # Hidden graph
        #################################################################
        adj, features = fea_to_graph(q)
        adj, features, graph_indicator = generate_batches(adj, features,
                                                          self.args.batch_size * self.args.clip_len,
                                                          self.args.device,
                                                          shuffle=False)
        rw_kernel = self.rw_gk(adj, features, graph_indicator)
        # y = F.one_hot(label, num_classes=400)

        criterion = nn.NLLLoss()

        loss_kernel = criterion(rw_kernel, label)
        ...
        logits = torch.log(rw_kernel + EPS).flatten(0, -2)
        #     loss = self.xent(logits, target).mean()
        acc = (torch.argmax(logits, dim=-1) == label).float().mean()
        diags.update({f"{H} kernel": loss_kernel.detach(),
                      f"{H} acc": acc})
        xents += [loss_kernel]
        ...
        # Visualizations
        #################################################################
        # if (np.random.random() < 0.02) and (self.utils is not None):  # and False:
        #     # if self.utils is not None:  # and False:
        #
        #     with torch.no_grad():
