...
class VISDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.folder='/home/qinzheyun/Data/YouTube/vis/2021/valid/JPEGImages/'
        self.ann_folder = '/home/qinzheyun/Data/YouTube/vos/valid/Annotations/'
        ann_path='/home/qinzheyun/Data/YouTube/vis/2021/valid/instances.json'
        self.videos = json.load(open(ann_path, 'rb'))['videos']

        vis_num = len(self.videos)
        for i in range(vis_num):
            self.jpgfiles.append(os.path.join(self.folder,
                                              self.videos[i]['file_names'][0][:10]))
            self.lblfiles.append(os.path.join(self.ann_folder,
                                              self.videos[i]['file_names'][0][:10]))
    ...
    def __getitem__(self, index):
        ...
        # img_paths, lbl_paths = self.make_paths(folder_path, None)
        ann_num = len(lbl_paths)


        lbl_path = lbl_paths[0]
        lblimg = cv2.imread(lbl_path)

        # cv2.imshow('mm', lblimg)

        if ann_num > 1:
            for j in range(ann_num-1):
                lbl_path = lbl_paths[j+1]
                lblimg_ = cv2.imread(lbl_path)
                lblimg += lblimg_
        ...
        #     for _ in range(lbls.shape[0] - self.videoLen):
