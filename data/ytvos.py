...
import torchvision
from utils.pycocotools.ytvos import YTVOS
from utils.pycocotools.ytvoseval import YTVOSeval
...
class YTVOSDataset:
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(target['boxes'])):
            ...
            # plt.show()
            ins_ann = ins_ann.permute(1,2,0).flatten().resize_(384).unsqueeze(0)
...
def build(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train": (root / "vos/train/JPEGImages", root / "utils/2019/train.json"),
        "val": (root / "vos/valid/JPEGImages", root / "utils/2019/valid.json"),
    }
    img_folder, ann_file = PATHS[image_set]
    dataset = YTVOSDataset(img_folder, ann_file, transforms=make_coco_transforms(image_set),
                           return_masks=args.ytvos_masks, num_frames = args.clip_len)
    return dataset
