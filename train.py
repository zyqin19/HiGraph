...
def train_one_epoch(model, optimizer, lr_scheduler, data_loader, device, epoch, print_freq,
    ...
    for step, (video, orig, label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        output, loss, diagnostics = model(video, label)
        loss = loss.mean()
...
def _get_cache_path(filepath):
    ...
    cache_path = os.path.join("~", ".torch", "vision", "datasets", "kinetics", h[:9] + ".pt")
...
def main(args):
    ...
    if args.data_parallel:
        model = torch.nn.parallel.DataParallel(model, device_ids=args.device_ids)
